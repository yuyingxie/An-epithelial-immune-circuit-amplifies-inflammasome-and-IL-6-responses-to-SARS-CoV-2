...
load('/mnt/research/xlab/covid.y/pbmc/original/cm.pp.14.rda')
load("/mnt/research/xlab/covid.y/pbmc/original/covid_combined.emat.rda")
load("/mnt/research/xlab/covid.y/pbmc/original/covid_combined.nmat.rda")
...
setwd('/mnt/research/xlab/covid.y/pbmc/original')
...
covid.markers.heatmap <- function(markers.matrix = NULL, color = c("blue", "white", "red"), 
                                  ...
                                  save = F, width = 7, height = 11, file = "/mnt/research/xlab/covid.y/pbmc/original/p.pdf") {
...
new.markers.heatmap <- function(markers.matrix = NULL, fontsize_row = 10, color.rows = T,
                                ...
                                file = "/mnt/research/xlab/covid.y/pbmc/original/p.pdf") {
...
myFeaturePlot <- function(object = NULL, features = features, ncol = NULL, save = T, save.as = "png", height = 7, width = 7, cols = c("lightgrey", "blue")) {
  ...
  if(save) {
    if(save.as=="pdf"){
      ggsave("p.pdf", path = "/mnt/research/xlab/covid.y/pbmc/original/", height = height, width = width)
    }
    if(save.as=="png"){
      ggsave("p.png", path = "/mnt/research/xlab/covid.y/pbmc/original/", height = height, width = width)
    }
  }
}
...
dpi = 300
setwd('/mnt/research/xlab/covid.y/pbmc/original')
path <- '/mnt/research/xlab/covid.y/pbmc/GSE150728_RAW_1/'
fnames <- dir(path)
files <- paste(path,fnames,sep='')
ufiles <- lapply(files, readRDS)


covid.names <- paste0('c',c(1:8))
healthy.names <- paste0('h',c(1:6))
ob.list <- c(covid.names, healthy.names)
names(ufiles) <- ob.list
...
write.table(singler.pred$labels,file = 'singler.pred$labels.txt')

###
png(file=paste('singler.pred.orig.ident.png',sep=''), width = dpi*16, height = dpi*8, units = "px",res = dpi,type='cairo')
print(plotScoreHeatmap(singler.pred, clusters = covid_combined@meta.data$orig.ident))
dev.off()

png(file=paste('singler.pred.seurat_clusters.png',sep=''), width = dpi*16, height = dpi*8, units = "px",res = dpi,type='cairo')
print(plotScoreHeatmap(singler.pred, clusters = covid_combined@meta.data$seurat_clusters))
dev.off()
...
write.table(singler.final,file = 'singler.final.txt')
covid_combined$singler <- singler.pred$labels

png(file=paste('covid_combined.groupbysingler.png',sep=''), width = dpi*16, height = dpi*8, units = "px",res = dpi,type='cairo')
...
save(covid_combined.nc,file = 'covid_combined.nc.rda')

png(file=paste('covid_combined.nc.png',sep=''), width = dpi*16, height = dpi*8, units = "px",res = dpi,type='cairo')
...
singler.results <- aggregate(count ~ ., singler.results, FUN = sum)
singler.final <- singler.results %>% group_by(cluster) %>% top_n(n = 1, wt = count)
write.table(singler.final,file = 'singler.final1.txt')
...
save(covid_combined.nc.markers,'covid_combined.nc.markers.rda')
...
ggsave("p.pdf", path = "/mnt/research/xlab/covid.y/pbmc/original/", height = 5, width = 10)
